// Dashboard Main - Goals Management
document.addEventListener('DOMContentLoaded', function() {
    // Load goals
    fetch(goalsIndexRoute)
        .then(r => r.json())
        .then(renderGoals)
        .catch(()=>{});
    ...
    if (aiBtn) aiBtn.addEventListener('click', function(){
            ...
            fetch(aiInsightRoute, {method: 'POST', headers: {'X-CSRF-TOKEN': csrfToken }})
                .then(r => r.json())
...
function toggleGoal(id){
    fetch(`${goalsBaseUrl}/${id}/toggle`, {method:'POST', headers:{'X-CSRF-TOKEN': csrfToken}})
        .then(()=>fetch(goalsIndexRoute).then(r=>r.json()).then(renderGoals));
}

function deleteGoal(id){
    if (!confirm('Delete this goal?')) return;
    fetch(`${goalsBaseUrl}/${id}`, {method:'DELETE', headers:{'X-CSRF-TOKEN': csrfToken}})
        .then(()=>fetch(goalsIndexRoute).then(r=>r.json()).then(renderGoals));
}
...
function submitGoal(){
    ...
    fetch(goalsStoreRoute, {method:'POST', headers:{'X-CSRF-TOKEN': csrfToken}, body: fd})
        .then(r=>r.json())
        .then(()=>{ form.reset(); bootstrap.Modal.getInstance(document.getElementById('goalModal')).hide(); return fetch(goalsIndexRoute) })
